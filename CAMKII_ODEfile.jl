module Module_CAMKII  ## uM/ms
    ...
    function morotti_camkiiODE!(du,u,p,t)
    ## State variables
        ...
        (LCC_PKAp, LCC_CKdyadp, RyR2809p, RyR2815p, PLBT17p, LCC_CKslp) = u

    ## Parameters
        (CaMKIIactDyad, LCCtotDyad, RyRtot, PP1_dyad, PP2A_dyad, OA, PLBtot,
         CaMKIIactSL, LCCtotSL, PP1_SL, PP1_PLB_avail) = p
    ...
    ## ODE EQUATIONS
        ...
        @variables LCC_CKdyadn(t) LCC_CKdyadn ~ LCCtotDyad - LCC_CKdyadp
        LCCDyad_PHOS = (k_ckLCC*CaMKIIactDyad*LCC_CKdyadn)/(KmCK_LCC+LCC_CKdyadn)
        @variables LCCDyad_DEPHOS(t) LCCDyad_DEPHO ~ (k_pp1LCC*PP1_dyad*LCC_CKdyadp)/(KmPP1_LCC+LCC_CKdyadp)*OA_PP1
        du[2] = LCCDyad_PHOS - LCCDyad_DEPHOS
        
        # CaMKII phosphorylation of Sub-sarcolemmal LCCs
        @variables LCC_CKsln(t) LCC_CKsln ~ LCCtotSL - LCC_CKslp
        LCCSL_PHOS = (k_ckLCC*CaMKIIactSL*LCC_CKsln)/(KmCK_LCC+LCC_CKsln)
        @variables LCCSL_DEPHOS(t) LCCSL_DEPHOS ~ (k_pp1LCC*PP1_SL*LCC_CKslp)/(KmPP1_LCC+LCC_CKslp)*OA_PP1
        du[6] = LCCSL_PHOS - LCCSL_DEPHOS
        
        # PKA phosphorylation (currently unused elsewhere)
        @variables LCC_PKAn(t) LCC_PKAn ~ LCCtotDyad - LCC_PKAp
        du[1] = (k_pkaLCC*PKAc*LCC_PKAn)/(KmPKA_LCC+LCC_PKAn) - (k_pp2aLCC*PP2A_dyad*LCC_PKAp)/(KmPP2A_LCC+LCC_PKAp)*OA_PP2A
        
        # RyR states
        @variables RyR2815n(t) RyR2815n ~ RyRtot - RyR2815p
        RyR_BASAL = kb_2815*RyR2815n
        RyR_PHOS = (k_ckRyR*CaMKIIactDyad*RyR2815n)/(KmCK_RyR+RyR2815n)
        @variables RyR_PP1_DEPHOS(t) RyR_PP1_DEPHOS ~ (k_pp1RyR*PP1_dyad*RyR2815p)/(KmPP1_RyR+RyR2815p)*OA_PP1
        @variables RyR_PP2A_DEPHOS(t) RyR_PP2A_DEPHOS ~ (k_pp2aRyR*PP2A_dyad*RyR2815p)/(KmPP2A_RyR+RyR2815p)*OA_PP2A
        du[4] = RyR_BASAL + RyR_PHOS - RyR_PP1_DEPHOS - RyR_PP2A_DEPHOS
        
        # PKA phosphorylation of Ser 2809 on RyR (currently unused elsewhere)
        @variables RyR2809n(t) RyR2809n ~ RyRtot - RyR2809p
        du[3] = kb_2809*RyR2809n + (k_pkaRyR*PKAc*RyR2809n)/(KmPKA_RyR+RyR2809n) - (k_pp1RyR*PP1_dyad*RyR2809p)/(KmPP1_RyR+RyR2809p)*OA_PP1
        
        # PLB states
        PP1_PLB = PP1_dyad*PP1_PLB_avail # Inhibitor-1 regulation of PP1_dyad included here
        @variables PLBT17n(t) PLBT17n ~ PLBtot - PLBT17p
        PLB_PHOS = (k_ckPLB*PLBT17n*CaMKIIactDyad)/(KmCK_PLB+PLBT17n)
        @variables PLB_DEPHOS(t) PLB_DEPHOS ~ (k_pp1PLB*PP1_PLB*PLBT17p)/(KmPP1_PLB+PLBT17p)*OA_PP1
        du[5] = PLB_PHOS - PLB_DEPHOS

        return du
    end

    export morotti_camkiiODE
