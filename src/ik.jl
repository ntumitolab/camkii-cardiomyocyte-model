...
function get_ik_sys(k_i, k_o, na_i, na_o, vm; IKUR_PKAp=0, name=:iksys)
    ...
    # IKr
    alphaa0 = 22.348Hz * exp(0.01176/mV * V)
    betaa0 = 47.002Hz * exp(-0.0631/mV * V)
    alphaa1 = 13.733Hz * exp(0.038198/mV * V)
    betaa1 = 0.0689Hz * exp(-0.04178/mV * V)
    alphai_mERG = 90.821Hz * exp(0.023391/mV * V)
    betai_mERG = 6.497Hz * exp(-0.03268/mV * V)
    ...
    return ODESystem([
            ...
            E_K ~ nernst(k_o, k_i),
            IK1 ~ GK1 * hil(k_o, 210μM) * (vk1/mV) / (0.1653 + exp(0.0319/mV * vk1)),
            sinf ~ expit((V + 31.97156mV) * inv(-4.64291mV)),
            rinf ~ expit((V - 3.55716mV) * inv(14.61299mV)),
            slowinf ~ sinf,
            taur ~ 1000ms / (45.16 * exp(0.03577/mV * (V + 50mV)) + 98.9 * exp(-0.1/mV * (V + 38mV))),
            taus ~ (350 * exp(-(((V + 70mV) * inv(15mV))^2)) + 35 - 26.9) * ms,
            tausslow ~ (3700 * exp(-(((V + 70mV) * (30mV))^2)) + 35 + 37.4) * ms,
            Ito ~ Gt * i_r * (f_is * i_s + (1 - f_is) * i_sslow) * (vm - E_K),
            D(i_r) ~ (rinf - i_r) / taur,
            D(i_s) ~ (sinf - i_s) / taus,
            D(i_sslow) ~ (slowinf - i_sslow) / tausslow,
            IKs ~ GKs * i_nKs^2 * (vm - E_K) * fracIKuravail * 2,
            alphan ~ 0.00000481333 / 0.128 * exprel(-0.128/mV * (V + 26.5mV)),
            betan ~ 0.0000953333 * exp(-0.038/mV * (V + 26.5mV)),
            ...
            D(i_OK) ~ rc2o - roi,
            D(i_IK) ~ roi,
            yinf ~ expit(-(V + 78.65mV) * inv(6.33mV)),
            tauy ~ 1000ms / (0.11885 * exp((V + 75mV) * inv(28.37mV)) + 0.56236 * exp(-(V + 75mV) * inv(14.19mV))),
