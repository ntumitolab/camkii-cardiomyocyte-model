...
function build_bar_sys(;
    ...
    rn = @reaction_network begin
        ...
        mm(PLBp, k_PP1_PLB, Km_PP1_PLB), PLBp => PLB
        mm(PLM, k_PKA_PLM * PKACI, Km_PKA_PLM), PLM => PLMp
        mm(PLMp, k_PP1_PLM, Km_PP1_PLM), PLMp => PLM
        mm(TnI, k_PKA_PLM * PKACI, Km_PKA_TnI), TnI => TnIp
        mm(TnIp, PP2A_TnI, Km_PP2A_TnI), TnIp => TnI

        # LCC modification
        mm(LCCa, k_PKA_LCC * (PKACII_LCCtot / PKAIItot) * PKACII, Km_PKA_LCC/epsilon), LCCa => LCCap
        mm(LCCap, k_PP2A_LCC, Km_PP2A_LCC/epsilon), LCCap => LCCa
        mm(LCCb, k_PKA_LCC * (PKACII_LCCtot / PKAIItot) * PKACII, Km_PKA_LCC/epsilon), LCCb => LCCbp
        mm(LCCbp, k_PP1_LCC, Km_PP1_LCC/ epsilon), LCCbp => LCCb

        # Ikur modification
        mm(KURn, k_pka_KUR * (PKAII_KURtot / PKAIItot) * PKACII, Km_pka_KUR/epsilon), KURn => KURp
        mm(KURp, k_pp1_KUR, Km_pp1_KUR/epsilon), KURp => KURn
    end
end
