...
const NaVsCaMKIIclamp = 0 # if 1, CaMKII Clamp on NaV
...
function get_ecc_equations()
    ...
    var_eqs = [
        ...
        PLM_PKAp ~ ifelse(PLMkoFlag == 1, 1, PLMp / PLMtotBA)
    ]
    ...
    @parameters IbarNaK = 5 * (-0.1NaGainFlag+1) * (-0.5DigitalisFlag+1) * (-0.2PLMkoFlag+1) * (-StrophFlag+1)        # [uA/uF] changed from rabbit (1.90719)
    ...
    @parameters IbarNCX = 1 * (1+0.5CKIIflag) # [uA/uF] changed from rabbit (9)
    ...
    @parameters Vmax_SRCaP = 1.15*1.15*2.86e-4*(1-CaffeineFlag) # [mM/msec] (mmol/L cytosol/msec) changed
    ...
    @parameters koCa = 10 * (1+6.5CaffeineFlag)                 # [mM^-2 1/ms]
    ...
    @variables fPKA_TnI(t) koff_tncl(t)
    PKA_eqs = [
    ...
    @variables ena_junc(t) ena_sl(t) ek(t) eca_junc(t) eca_sl(t) ecl(t) delta_loop(t) deltGbarNal_CKII(t)
    ...
    @variables am(t) bm(t) bh(t) bj(t) ah(t) aj(t) I_Na_junc1(t) I_Na_sl1(t) GNaB(t)

    INa_fast_eqs = [
        delta_loop ~ ifelse(NaVsCaMKIIclamp == 1, (3/(RyRp_OE_mean-RyRp_WT_mean)) * RyRp_OEloop_min - (3/(RyRp_OE_mean-RyRp_WT_mean)) * RyRp_WT_mean, (3/(RyRp_OE_mean-RyRp_WT_mean)) * RyR_CKp - (3/(RyRp_OE_mean-RyRp_WT_mean)) * RyRp_WT_mean),
        GNaB ~ ifelse(loop == 1, (4.5) * 0.297e-3 * (1+delta_loop), 4.5 * 0.297e-3*(3NaGainFlag+1)*(1.4PLMkoFlag+1)),
        # Max INa alterations with CaMKII hyperactivity as in Hund & Rudy 2008
        deltGbarNal_CKII ~ ifelse(loop == 1, delta_loop*(NaGainFlag)*(CKIIflag), 3*(NaGainFlag)*(CKIIflag)),
    ...
    @variables hlss(t) I_Nalj(t) I_Nalsl(t) I_Na_junc2(t) I_Na_sl2(t) I_Na_junc(t) I_Na_sl(t) GbarNal(t)
    ...
    @variables fnak(t) I_nak_junc(t) I_nak_sl(t) I_nak(t) KmNaip_PKA(t) KmNaip(t)
    ...
    @variables xurss(t) yurss(t) tauxur(t) tauxur2(t) tauyur1(t) tauyur2(t) I_kur1(t) I_kur2(t) fracIKuravail(t) I_kur(t)

    # I_ks: Slowly Activating K Current
    @parameters I_ks_junc = 0       # No IKs in mouse
    @parameters I_ks_sl = 0         # No IKs in mouse
    @parameters I_ks = I_ks_junc+I_ks_sl
    @parameters fracIKuravail = (1-a_Kur)+a_Kur*(IKur_PKAp/fracIKurp0)
    ...
    @parameters Py = ifelse(CKIIflag == 0, 182, 15)
    @parameters Pr1 = ifelse(CKIIflag == 0, 8085, 3600)
    @parameters Pr2 = ifelse(CKIIflag == 0, 313, 500)       # Normal
    ...
    @variables fCKII_PLB(t) fPKA_PLB(t) J_serca(t) Kmf(t)
    ...
    @variables I_Na_tot_junc(t) I_Na_tot_sl(t) I_Ca_tot_junc(t) I_Ca_tot_sl(t) junc_sl(t) sl_junc(t) sl_myo(t) myo_sl(t) I_K_tot(t)
    ...
    Ion_eqs = [
        ...
        D(Nai) ~ ifelse(NaClampFlag == 1, 0, J_na_slmyo/Vmyo*(Nasl-Nai)),                                       # Na clamp  du[34]
        # K Concentration
    ...
    #return sys
    return vcat(var_eqs, PKA_eqs, INa_fast_eqs, INaL_eqs, INa_eqs, I_nabk_eqs, I_nak_eqs, IK_eqs, Ito_eqs, Ik1_eqs, I_ClCa_eqs, ICa_HH_eqs, ICaMar_m1j_eqs,
    ICaMar_m2j_eqs, ICaMar_m1sl_eqs, ICaMar_m2sl_eqs, I_ncx_eqs, ICabp_eqs, RyR_eqs, SERCA_eqs, Buffering_eqs, Ion_eqs, AP_eqs, MemPot_eqs)
end
